	...
	bool Texture::Load(bool a_resizeToScreenRes)
	{
		...
		if (SUCCEEDED(hr)) {
			if (auto renderer = RE::BSGraphics::Renderer::GetSingleton()) {
				...
				hr = DirectX::CreateTexture(reinterpret_cast<ID3D11Device*>(RENDERER_DATA(renderer).forwarder), image->GetImages(), 1, image->GetMetadata(), &pTexture);
				...
				if (SUCCEEDED(hr)) {
					...
					hr = reinterpret_cast<ID3D11Device*>(RENDERER_DATA(renderer).forwarder)->CreateShaderResourceView(pTexture.Get(), &srvDesc, &srView);
					result = SUCCEEDED(hr);
				}
